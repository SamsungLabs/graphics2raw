...
import data_preparation.data_generator_neural_isp as dg

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


class DatasetRAW(object):
    def __init__(self, root, batch_size, patch_size, stride, wb_illum, on_cuda, which_input, is_PS_sRGB,
                 data_mode='train'):
        ...
        rawimgs = dg.datagenerator_raw(data_dir=os.path.join(root, which_input),
                                       ...
                                       wb_illum=wb_illum, batch_size=batch_size,
                                       patch_size=patch_size, stride=stride,
                                       is_PS_sRGB=is_PS_sRGB, data_mode=data_mode)
        ...
        srgbimgs = dg.datagenerator_sRGB(data_dir=os.path.join(root, 'clean'), batch_size=batch_size,
                                         patch_size=patch_size, stride=stride, data_mode=data_mode)
...
if __name__ == '__main__':
    ...
    wb_illum = 'asn'
    on_cuda = False
    which_input = 'clean_raw'

    image_datasets = {
        x: DatasetRAW(os.path.join(data_dir, x), batch_size, patch_size, stride, wb_illum, on_cuda, which_input,
                      is_PS_sRGB=True, data_mode=x)
        for x in ['val']}
